...
class MainWindow(QMainWindow):
    def __init__(self, username, username_id):
        ...
        self.clicked_date = None      # Connect instruct_button to open_user_manual method
    ...
    def highlight_deadlines(self):
        format = QTextCharFormat()
        format.setBackground(QBrush(QColor("lightblue")))
        format.setForeground(QBrush(QColor("red")))
        data = self.db_manager.populate_deadline()
        dates = [item[2] for item in data]
        for deadline in dates:
            self.ui.calendarWidget.setDateTextFormat(deadline, format)
    ...
    def draw_header_footer(self, c, width, height):
        ...
        footer_y_position = 40  # Adjust as needed for bottom margin
        c.setFont('Helvetica', 12)
        c.drawString(30, footer_y_position, f"Generated by: {self.username} / {self.username_id}")
        c.drawRightString(width - 30, footer_y_position, "Generated on: " + pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"))
